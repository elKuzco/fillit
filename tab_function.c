...
/*   Updated: 2018/12/10 21:30:15 by qlouisia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
  ...
  while (lst->str[i])
  {
    ...
    else
    {
      if(lst->str[i] <= lst->str[i - 1])
        tab_y++;
    tab_x = lst->str[i] - '0';
    if (!check_place(x + tab_x, y + tab_y, tab))
  //      return(clean_piece(x,y,..);   
